...
`.claude/settings.json` регистрирует `PreToolUse`-хук `.claude/hooks/protected.sh`:
он блокирует запись через Edit/Write в `.env`, `.env.local`, `prisma/migrations`
и `package-lock.json` (выход с кодом 2 = отмена вызова). Настройка проектная и
коммитится, так что защита действует у всех, кто склонировал репозиторий.

Если файл из этого списка нужно поменять — править вручную в терминале, а не
через агента. Список паттернов — в самом скрипте.
